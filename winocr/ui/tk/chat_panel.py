...
FILE_TYPES = [
    ("所有支持", "*.png *.jpg *.jpeg *.bmp *.gif *.tiff *.webp "
...
class ChatPanel:
    ...
    def _build_header(self) -> None:
        ...
        tk.Label(h, text="🤖", font=("Microsoft YaHei", 17),
                 bg=theme.ACCENT, fg="white").pack(side=tk.LEFT, padx=(10, 6))
        box = tk.Frame(h, bg=theme.ACCENT)
        box.pack(side=tk.LEFT, fill=tk.Y, pady=4)
        tk.Label(box, text="WinOCR AI", font=theme.TITLE_FONT,
                 bg=theme.ACCENT, fg="white").pack(anchor=tk.W)
        self.sub_label = tk.Label(box, text="解读识别结果 · 看图 · 读文档",
                                  font=theme.UI_FONT_SMALL,
                                  bg=theme.ACCENT, fg=theme.ACCENT_SUBTITLE)
        self.sub_label.pack(anchor=tk.W)
        tk.Button(h, text="收起", command=self.window.toggle_chat,
                  bg=theme.ACCENT, fg="white", activebackground=theme.ACCENT_HOVER,
                  activeforeground="white", relief=tk.FLAT, bd=0,
                  font=theme.UI_FONT_SMALL, cursor="hand2").pack(side=tk.RIGHT, padx=8)
    ...
    def _build_options(self) -> None:
        ...
        ttk.Checkbutton(bar, text="带上识别原文",
                        variable=self.var_use_text).pack(side=tk.LEFT)
        ttk.Checkbutton(bar, text="带上最近截图",
                        variable=self.var_use_shot).pack(side=tk.LEFT, padx=(10, 0))

        ttk.Button(bar, text="清空对话", width=9,
    ...
    def _build_attachments(self) -> None:
        ...
        ttk.Button(top, text="添加文件", width=9,
                   command=self.open_file_dialog).pack(side=tk.LEFT, padx=(6, 0))
    ...
    def _build_input(self) -> None:
        ...
        self.input = tk.Text(holder, height=2, wrap=tk.WORD, font=theme.UI_FONT,
                             bg=theme.INPUT_BG, fg=theme.TEXT_HINT, relief=tk.FLAT, bd=1,
    ...
    def _on_canvas_resize(self, event) -> None:
        ...
        for b in self.bubbles:
            # user 气泡是 Label（用 wraplength）；assistant/system 气泡是外层 Frame（用 width 控 Text 换行）
            try:
    ...
    def add_bubble(self, role: str, text: str) -> None:
        ...
        common = dict(wraplength=self._wrap_len(), justify=tk.LEFT, font=theme.UI_FONT,
                      bg=colors["bg"], fg=colors["fg"],
                      padx=theme.BUBBLE_PAD, pady=theme.BUBBLE_PAD,
                      relief=tk.FLAT, bd=0)
        if role == "user":
            bubble = tk.Label(outer, text=text, highlightthickness=0, **common)
            bubble.pack(anchor=tk.E, padx=4)
        else:
            row = tk.Frame(outer, bg=theme.CANVAS_BG)
            row.pack(anchor=tk.W, fill=tk.X)
            if role == "assistant":
                tk.Label(row, text="🤖", font=("Microsoft YaHei", 13),
                         bg=theme.CANVAS_BG, fg=theme.ACCENT
            ...
            txt = tk.Text(border, wrap=tk.WORD,
                          font=theme.UI_FONT, bg=colors["bg"], fg=colors["fg"],
                          padx=theme.BUBBLE_PAD, pady=theme.BUBBLE_PAD,
            ...
            # 最低 2 行高度：哪怕 reply 是空字符串或只有 1 行，气泡也至少有可见区域
            lines = int(txt.index("end-1c").split(".")[0])
            txt.config(height=max(2, lines))
    ...
    def _show_thinking(self) -> None:
        ...
        row.pack(anchor=tk.W, fill=tk.X)
        tk.Label(row, text="🤖", font=("Microsoft YaHei", 13),
                 bg=theme.CANVAS_BG, fg=theme.ACCENT).pack(side=tk.LEFT, padx=(0, 4), anchor=tk.N)
        tk.Label(row, text="WinOCR 正在思考…", font=theme.UI_FONT,
                 bg=theme.ASSIST_BG, fg=theme.TEXT_MUTED,
                 padx=theme.BUBBLE_PAD, pady=theme.BUBBLE_PAD, relief=tk.FLAT, bd=0,
                 highlightbackground=theme.ASSIST_BORDER, highlightthickness=1
    ...
    def _render_chips(self) -> None:
        ...
        if not self.attachments and not self.pending:
            ttk.Label(self.attach_inner,
                      text="（可粘贴 / 拖拽图片或文档，也可点「添加文件」）",
                      foreground=theme.TEXT_HINT, font=theme.UI_FONT_SMALL).pack(anchor=tk.W)
            return
        ...
        for name in self.pending:
            chip = ttk.Frame(row, relief=tk.SOLID, borderwidth=1)
            chip.pack(side=tk.LEFT, padx=2, pady=2)
            ttk.Label(chip, text=f"⏳ {name}", foreground=theme.TEXT_MUTED
    ...
    def _tooltip(widget, text: str) -> None:
        ...
        def show(_e):
            ...
            tk.Label(win, text=text, bg="#ffffe0", fg="#333333",
                     font=theme.UI_FONT_SMALL, relief=tk.SOLID, bd=1,
                     padx=4, pady=2, wraplength=320, justify=tk.LEFT).pack()
            tip["win"] = win
    ...
    def add_path(self, path: str) -> None:
        ...
        def _err(e):
            self.ui.post(self._on_parsed, name,
                         Attachment(kind="text", path=path, error=str(e)))

        self.app.pipeline.run_async(self.app.pipeline.parse_attachment, path,
    ...
    def send(self) -> None:
        ...
        ai = self.app.services.get("ai")
        if ai is None or not ai.available():
            messagebox.showinfo("AI 未就绪",
                                "请先在「API 设置」中填入 GLM API Key。",
                                parent=self.root)
            return
        if self.pending:
        ...
        self.app.pipeline.run_async(self.app.pipeline.chat, msg,
                                    on_done=_done, on_error=_err)
    ...
    def _on_reply(self, gen: int, reply: str) -> None:
        ...
        if gen != self._gen:                 # 已被「清空对话」或新消息作废
            ...
            return
        self._finish()
        self.add_bubble("assistant", reply)
        # 强制画布刷新 + 滚到底（add_bubble 内的 _scroll_bottom 在某些时序下 canvas
        # bbox 还没更新，yview_moveto 会失效——这里再补一次 update_idletasks）
        try:
            self.canvas.update_idletasks()
            self.canvas.yview_moveto(1.0)
        except Exception:
            pass
        self.window.set_status(f"✓ AI 已回复（{len(reply)} 字）")
