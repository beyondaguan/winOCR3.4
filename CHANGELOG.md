...
**llama.cpp 换模型：保存设置后首次翻译/对话干等模型加载（表现为「切换延迟」）**
- 根因：`set_config` / `apply_config` 换模型时只清空缓存（`_llm=None`），加载被推迟到下一次推理调用内同步执行，首段输出前多等数秒
- 修复：会话中真实换模型时立即起后台守护线程预加载；预热线程与推理共用实例锁天然串行（遵守「原生层禁止并发」铁律）；多次切换时各线程在锁上收敛，后到者发现目标模型已加载即退出、不重复装载
- 附带修复：加载中再换名的窄竞态——`_get_llm` 以「发起加载时的模型名快照」（`_loaded_for_name`）登记已加载实例，名字不匹配自动重载，不会拿旧模型冒充新模型
- 文件：`winocr/services/translate/llama_cpp.py`、`winocr/services/ai/llama_cpp_chat.py`

**应用启动后首次翻译同样要等模型加载**
- 根因：启动预热只覆盖 OCR 与 TTS，翻译引擎保持完全惰性
- 修复：`_warmup_async` 增加「当前选中的翻译引擎」预热（在线引擎 warmup 为空操作，无副作用），启动后第一次翻译即秒回
...
| 文件 | 改动 |
|------|------|
| `winocr/services/translate/llama_cpp.py` | 换模型后台预加载 + `_loaded_for_name` 防换名竞态 |
| `winocr/services/ai/llama_cpp_chat.py` | 对话提供方同款预加载与竞态防护 |
| `winocr/ui/tk/app.py` | 启动预热增加当前选中的翻译引擎 |
| `winocr/version.py` | 3.4.27 → 3.4.28 |
| `README.md` | 版本号同步 |
| `CHANGELOG.md` | 本节 |

### 验证

- 临时脚本实测（用后已删）：启动装配保持惰性不预热；会话内换模型后台预加载 1.4~1.6s；切换完成后首译 0.6s 出结果；快速连切两轮后模型收敛正确；对话提供方预加载后 chat 正常
- `python -m compileall winocr` 零错误
- `pytest tests -q -k "translate or ai or config or dispatcher"`：47 通过 / 2 失败（`test_sapi_host`、`test_tts_fallback`，既有 SAPI 环境问题，与本版改动无交集）
- 硬件基线备忘（R5 5500 / 16GB，6 线程 ctx2048 纯 CPU）：HY-MT1.5-1.8B-Q4_K_M ≈ 15.5 tok/s（加载后 ~2.0GB）；Qwen2.5-0.5B-Instruct-Q4_K_M ≈ 44 tok/s（~540MB）
