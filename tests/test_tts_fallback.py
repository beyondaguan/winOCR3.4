...
def test_auto_edge_hit_stops_no_sapi():
    ...
    svc = _make("auto")
    calls = {"sapi": 0}
    svc._speak_edge = lambda t, c: True
    svc._speak_sapi = lambda t, c: calls.__setitem__("sapi", calls["sapi"] + 1) or True
    msgs = []
...
def test_auto_edge_fail_falls_back_to_sapi():
    ...
    svc = _make("auto")
    calls = {"edge": 0}
    svc._speak_edge = lambda t, c: calls.__setitem__("edge", calls["edge"] + 1) or False
    svc._speak_sapi = lambda t, c: True
    msgs = []
    svc._work("你好，世界。", msgs.append, threading.Event())
    assert calls["edge"] == 1
...
def test_edge_mode_fail_no_sapi():
    ...
    svc = _make("edge")
    calls = {"sapi": 0}
    svc._speak_edge = lambda t, c: False
    svc._speak_sapi = lambda t, c: calls.__setitem__("sapi", calls["sapi"] + 1) or True
    msgs = []
...
def test_sapi_mode_skips_edge():
    ...
    svc = _make("sapi")
    calls = {"edge": 0}
    svc._speak_edge = lambda t, c: calls.__setitem__("edge", calls["edge"] + 1) or True
    svc._speak_sapi = lambda t, c: True
    msgs = []
    svc._work("你好。", msgs.append, threading.Event())
    assert calls["edge"] == 0
...
def test_auto_all_fail_reports_failure():
    ...
    svc = _make("auto")
    svc._speak_edge = lambda t, c: False
    svc._speak_sapi = lambda t, c: False
    msgs = []
    svc._work("你好。", msgs.append, threading.Event())
    assert svc.last_engine == ""
