...
用法：
    python tools/download_ocr_model.py [small|medium]   # 默认 medium

说明：
  - small 模型 rapidocr pip 包已自带（PP-OCRv6_det_small.onnx 等），本脚本可跳过；
  - medium 包内没有，需从 ModelScope（RapidAI/RapidOCR，官方仓库）下载到 models/v6_medium/。
...
def main() -> int:
    ...
    if tier not in ("small", "medium"):
        print("用法: python tools/download_ocr_model.py [small|medium]")
        return 2
